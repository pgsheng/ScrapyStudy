...
class FileUtils(object):
    ...
    def write_friend(self, path, name):
        try:
            data = {'launch': {'friend': name}}
            # a追加写入，w覆盖写入
            with open(path, 'w', encoding='utf-8') as file:
                yaml.dump(data, file)
                self.log.info('写入已发好友名称列表到文件成功')
                return True
        except Exception as e:
            self.log.error('写入数据出现异常：%s' % e)
            return None

    def read_friend(self, path):
        try:
            with open(path, 'r', encoding='utf-8') as file:
                launch = yaml.load(file)
                friend_list = launch['launch']['friend']
                return friend_list
        except Exception as e:
            # self.log.info('读取的文件或key不存在，返回空')
            return list()
        ...
        try:
            with open(path, 'r', encoding='utf-8-sig') as file:
                ...
                if len(message_list) > 0:
                    ...
                    if len(msg_list) > 0:
                        return msg_list
                self.log.info('读取账号和密码为空')
                return None
        except Exception as e:
            self.log.error('读取获取账号和密码信息出现异常：%s' % e)
            return None
